...
import { FiClock, FiUsers, FiBarChart2, FiCheckCircle, FiUser, FiLayers, FiPlay } from 'react-icons/fi';
...
export default function CourseDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/courses/${params.id}/enroll`, {
        method: 'POST',
        headers: {
    ...
    return (
      <MainLayout>
        <div className="container mx-auto px-4 py-16 text-center md:px-8">
          ...
          <p className="mb-8">Kursus yang Anda cari tidak tersedia atau telah dihapus.</p>
          <Link href="/courses">
      ...
      <section className="bg-gradient-to-b from-primary-50 to-white py-12 dark:from-neutral-800 dark:to-neutral-900">
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid gap-8 lg:grid-cols-2">
            <div>
              <h1 className="mb-4 font-poppins text-3xl font-bold md:text-4xl">{course.title}</h1>
              <p className="mb-6 text-lg text-neutral-600 dark:text-neutral-300">{course.description}</p>
              
              <div className="mb-6 flex flex-wrap gap-4">
                <div className="flex items-center text-sm text-neutral-600 dark:text-neutral-400">
                  <FiUser className="mr-1" />
                  <span>Instruktur: <span className="font-medium text-neutral-900 dark:text-white">{course.instructor.full_name}</span></span>
                </div>
                <div className="flex items-center text-sm text-neutral-600 dark:text-neutral-400">
                  <FiClock className="mr-1" />
                  <span>Durasi: <span className="font-medium text-neutral-900 dark:text-white">{course.duration_hours || 0} jam</span></span>
                </div>
                <div className="flex items-center text-sm text-neutral-600 dark:text-neutral-400">
                  <FiUsers className="mr-1" />
                  <span>Siswa: <span className="font-medium text-neutral-900 dark:text-white">{course.enrollmentCount || 0}</span></span>
                </div>
                <div className="flex items-center text-sm text-neutral-600 dark:text-neutral-400">
                  <FiBarChart2 className="mr-1" />
                  <span>Level: <span className="font-medium text-neutral-900 dark:text-white">{course.level || 'Pemula'}</span></span>
                </div>
              </div>
          ...
          {activeTab === 'overview' && (
            <div className="grid gap-8 lg:grid-cols-3">
              <div className="lg:col-span-2">
                ...
                <h3 className="mb-3 mt-8 text-xl font-bold">Apa yang akan Anda pelajari</h3>
                <ul className="space-y-2 text-neutral-700 dark:text-neutral-300">
                  {course.modules && course.modules.slice(0, 4).map((module: any) => (
              ...
              <div>
                <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
                  <h3 className="mb-4 text-lg font-semibold">Kursus ini mencakup:</h3>
                  ...
                  <div className="mt-6">
                    {course.enrollmentStatus?.enrolled ? (
                      <Link href={`/dashboard/courses/${course.id}`}>
                        <Button variant="primary" fullWidth className="rounded-lg">
                          <FiPlay className="mr-2" /> Lanjutkan Belajar
                        </Button>
                      </Link>
                    ) : (
              ...
              {course.modules && course.modules.length > 0 ? (
                <div className="space-y-4">
                  {course.modules.map((module: any, index: number) => (
                    <div key={module.id} className="rounded-lg border border-neutral-200 bg-white overflow-hidden dark:border-neutral-700 dark:bg-neutral-800">
                          ...
                          {module.lessons.map((lesson: any, lessonIndex: number) => (
                            <li 
                              ...
                              className={`border-b border-neutral-200 p-4 last:border-b-0 dark:border-neutral-700 ${
                                lessonIndex % 2 !== 0 ? 'bg-neutral-50 dark:bg-neutral-800/30' : ''
                              }`}
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center">
                                  {lesson.content_type === 'video' && <FiPlay className="mr-2 text-primary-400" />}
                                  {lesson.content_type === 'quiz' && <FiBarChart2 className="mr-2 text-coral-400" />}
                                  {lesson.content_type === 'article' && <FiLayers className="mr-2 text-mint-400" />}
            ...
            <div>
              <div className="flex flex-col items-center space-y-4 sm:flex-row sm:items-start sm:space-x-6 sm:space-y-0">
                <div className="h-24 w-24 overflow-hidden rounded-full">
                  <Image
                    src={course.instructor.avatar_url || "/images/avatar-placeholder.jpg"}
                ...
                <div>
                  <h2 className="mb-2 text-2xl font-bold">{course.instructor.full_name}</h2>
                  <p className="mb-4 text-neutral-600 dark:text-neutral-300">
                    {course.instructor.role === 'instructor' ? 'Instruktur' : 'Admin'}
                  </p>
                  <p className="text-neutral-600 dark:text-neutral-300">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam in metus euismod, elementum nunc vel, feugiat magna. 
                    Suspendisse potenti. In ut lectus nec nisi malesuada varius ac ut diam.
                  </p>
                </div>
              </div>
            </div>
