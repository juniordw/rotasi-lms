const { User, Enrollment, Certificate, Course, Progress, Lesson, Module, Notification } = require('../models');
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const { full_name, email, username, department, role, avatar_url } = req.body;
  ...
  try {
    ...
    if (user.role === 'instructor') {
      ...
      if (courseCount > 0) {
        return res.status(400).json({
          success: false,
          message: 'User tidak dapat dihapus karena memiliki kursus. Harap transfer kursus ke instructor lain terlebih dahulu'
        });
      }
    }
  ...
  exports.getUserDashboardStats = async (req, res) => {
    try {
      ...
      if (req.user.role === 'student') {
        ...
        // Total waktu belajar
        const totalStudyTime = await Progress.sum('time_spent_minutes', {
          include: [
            {
              model: Enrollment,
              as: 'enrollment',
